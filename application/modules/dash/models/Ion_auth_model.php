...
class Ion_auth_model extends CI_Model
{
	...
	public function __construct()
	{
		...
		$this->nisn_column = $this->config->item('nisn', 'ion_auth');
	 ...
	 * @author    Mathew
	 */
	public function loginFromMobile($identity, $password, $remember=FALSE)
	{
		$this->trigger_events('pre_login');
		...
		$query = $this->db->select($this->identity_column . ', email, id, active, password, last_login, username, first_name, last_name')
						  ->where($this->identity_column, $identity)
						  ->limit(1)
						  ->order_by('id', 'desc')
						  ->get($this->tables['users']);
	 ...
	 * @author    Mathew
	 */
	public function loginSiswaFromMobile($nisn, $password, $remember=FALSE)
	{
		$this->trigger_events('pre_login');
		...
		$query = $this->db->select($this->nisn_column . ', email, id, active, password, last_login, username, first_name, last_name')
						  ->where($this->nisn_column, $nisn)
						  ->limit(1)
						  ->order_by('id', 'desc')
						  ->get($this->tables['users']);

		if ($this->is_max_login_attempts_exceeded($nisn))
		{
			// Hash something anyway, just to take up time
			$this->hash_password($password);
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($this->verify_password($password, $user->password, $nisn))
			{
				if ($user->active == 0)
				{
					$this->trigger_events('post_login_unsuccessful');
				...
				$this->clear_login_attempts($nisn);
				$this->clear_forgotten_password_code($nisn);
				...
				// Rehash if needed
				$this->rehash_password_if_needed($user->password, $nisn, $password);
		...
		$this->increase_login_attempts($nisn);
	...
	public function user_group_id($id)
	{
		...
		$this->db->join($this->tables['groups'], $this->tables['users_groups'] .'.group_id = '.$this->tables['groups'].'.id');
	...
	public function tes(){
		echo "sjdgshcsdlcbhds";
	}
	public function user_head_privilege($gp_id)
	{
		$this->db->select('*');
		$this->db->from($this->tables['groups']);
		$this->db->join($this->tables['group_perm'] , $this->tables['group_perm'].'.group_id = '.$this->tables['groups'].'.id');
		$this->db->join($this->tables['permissions'] , $this->tables['permissions'].'.perm_id = '.$this->tables['group_perm'].'.perm_id');
		$this->db->where($this->tables['permissions'].'.level', 0);
		$this->db->group_by($this->tables['permissions'].'.perm_id');
		$this->db->where($this->tables['group_perm'].'.group_id',$gp_id);
		$query = $this->db->get();
		return $query->result();
	}
	...
	public function user_sub_privilege($gp_id,$perm_id)
	{
		...
		$this->db->join($this->tables['group_perm'], $this->tables['group_perm'].'.group_id = '.$this->tables['groups'].'.id');
		$this->db->join($this->tables['permissions'], $this->tables['permissions'].'.perm_id = '.$this->tables['group_perm'].'.perm_id');
		$this->db->where($this->tables['permissions'].'.parent_id', $perm_id);
		$this->db->where($this->tables['permissions'].'.level', 1);
		$this->db->group_by($this->tables['permissions'].'.perm_id');
		$this->db->where($this->tables['group_perm'].'.group_id',$gp_id);
		$query = $this->db->get();
		return $query->result();
	...
	public function group_all_permission_by_group_id($gp_id=null){

		$this->db->select('*');
		$this->db->from($this->tables['group_perm']);
		// $this->db->join($this->tables['group_perm'], $this->tables['group_perm'].'.group_id = '.$this->tables['groups'].'.id');
		// $this->db->join($this->tables['permissions'], $this->tables['permissions'].'.perm_id = '.$this->tables['group_perm'].'.perm_id',"right");
		// $this->db->where($this->tables['permissions'].'.parent_id', $perm_id);
		// $this->db->where($this->tables['permissions'].'.level', 1);
		// $this->db->group_by($this->tables['permissions'].'.perm_id');
		if($gp_id != null){
		...
		// $this->db->order_by( $this->tables['permissions'].'.perm_name'.' asc, '.$this->tables['permissions'].'.urutan'.' asc'); 
		$this->db->order_by($this->tables['permissions'].'.urutan','asc');
		$this->db->order_by($this->tables['permissions'].'.perm_name','asc');
	...
	public function group_all_permission_by_group_in_id($gp_id=null){
		...
		// $this->db->group_by($this->tables['permissions'].'.perm_id');
		if($gp_id != null){
			$this->db->where($this->tables['group_perm'].'.group_id in ('.implode(", ",$gp_id).')' );
			$this->db->join($this->tables['permissions'], $this->tables['permissions'].'.perm_id = '.$this->tables['group_perm'].'.perm_id');
