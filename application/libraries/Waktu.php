...
class Waktu
{
    public function __construct(){
        ...
        // $ses_thn = $this->session->userdata('tahun');
        // $this->CI->db_sikd = $this->CI->load->database($ses_thn, TRUE);
    }

    function expJwt($time,  $exp_date_long){
        $now = new DateTime();
		$date = new DateTime(date('Y-m-d',$time));

		$dif = $date->diff($now)->format("%d");

        return $dif;

        // if($exp_date_long > $dif){
        //     return false;
        // }else{
        //     return true;
        // }
    ...
    function accept_post($field, $post)
    {
    ...
    function accept_parm($field, $parm)
    {
        $key_par = array_keys($parm);
        foreach($key_par as $key => $val)
        {
            if (in_array ($val, $field)) {
                $data[$val]  = $parm[$val];
            }
    ...
    {
        $id =  $this->CI->session->userdata('user_id');
        $nama = $this->CI->db_sikd->query('SELECT * FROM groups
